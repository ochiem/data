...
const dexOrder = ['Lifi','KyberSwap', 'Matcha','OKXDEX', 'ODOS','ParaSwap'];
const chainCodeMap = {
            bsc: 56,
            ethereum: 1,
            polygon: 137,
    ...
    bindEvents() {
      ...
      //  Save token button
        ...
        $('#exportTokensBtn').on('click', () => {
            ...
            this.logAction(`EXPORT DATA KOIN`);
            link.click();
        });
   ...
   // Token management
    ...
    loadTokenTable() {
        ...
        sortedTokens.forEach((token, index) => {
            // 🔽 CEX Badges + Status WD/DP/fee untuk token dan pair
            const cexBadges = token.selectedCexs.map(cex => {
                ...
                const formatStatus = (value, openText, closeText) => {
                    if (value === true) return `<strong class="text-success">${openText}</strong>`;
                    if (value === false) return `<strong class="text-danger">${closeText}</strong>`;
                ...
                const statusText = `
                    <span class="text-dark" title="Status Withdraw & Deposit">
                        <strong>${tokenSymbol}</strong> [${wdToken}|${dpToken}]~
                        <strong>${pairSymbol}</strong> [${wdPair}|${dpPair}]
                    </span>
                `;

                return `
                    <span class="badge ${this.getBadgeColor(cex, 'cex')} exchange-badge">${cexUpper}:</span><span class="badge fs-8 biru">${statusText} </span>                   
                `;
            }).join('<br/>');
            ...
            tbody.append(`
                <tr>
                    ...
                    <td>
                        ...
                        <br><small class="text-muted">${this.shortenAddress(token.contractAddress)} <b>[${token.decimals}]</b> / ${this.shortenAddress(token.pairContractAddress)} <b>[${token.pairDecimals}]</b></small>
                    </td>
                    <td>${chainBadge}</td>
    ...
    async CheckPrices() {
        ...
        const activeTokens = this.tokens
            .filter(t => t.isActive && this.selectedChains.includes(t.chain))
            .flatMap(token =>
                token.selectedCexs.map(cex => ({
                    ...token,
                    cexName: cex
                }))
            )
       ...
       // console.warn("DATA TOKEN PAIR: ",activeTokens);    
        if (activeTokens.length === 0) {
        ...
        const tokenBatches = chunkArray(activeTokens, tokensPerBatch);

        // 🕒 Mulai timer
        const startTime = new Date();
        const startStr = startTime.toLocaleTimeString();
        $('#scanTimeInfo').html(`<span class="text-dark">&nbsp;🕒 Start: ${startStr}</span>&nbsp;`);
        ...
        $('#scanTimeInfo').append(`<span class="text-primary">&nbsp;&nbsp;✅ Durasi: ${minutes}:${seconds}</span>&nbsp;`);
       
        this.showAlertWithAudio();
        // Disable filter chain checkbox
    ...
    async CheckPricesSCROLL() {
      ...
      //  const settings = this.loadSettings();
        ...
        $('#scanTimeInfo').html(`<span>&nbsp;🕒 Mulai: ${startStr}</span>&nbsp;`);

            for (const batch of tokenBatches) {
              //console.log(batch);
              // ⬇️ Scroll ke baris pertama dari batch
                const firstToken = batch[0];
                this.tryAutoScrollToRow(firstToken); // hanya scroll jika centang
                   ...
                   // $('#scanProgressText').text(`Progress: ${currentIndex} dari ${totalTokens}`);
                    ...
                    for (const cexName of token.selectedCexs) {
                        ...
                        const dexPromises = token.selectedDexs.map(async (dexName) => {
                            ...
                            await this.fetchDEXPrices(token, priceData, dexName, cexName, 'cex_to_dex');
                               // $('#scanProgressPercent').text(`${percent}%  ${(cexName).toUpperCase()} vs ${(dexName).toUpperCase()} on ${token.chain}`);
                                $('#scanProgressText').text(`Checking ${(cexName).toUpperCase()} vs ${(dexName).toUpperCase()} on ${token.chain}`);
                            await delay(500); // ⏱️ jeda antar arah pada DEX yang sama
            ...
            $('#scanTimeInfo').append(`<span>&nbsp;&nbsp;✅ Durasi: ${minutes}:${seconds}</span>&nbsp;`);
            this.showAlert('Scanner selesai.', 'warning');
    }
    ...
    async fetchDEXPrices(token, tokenPriceData, dexName, cexName, direction) {
        ...
        if (!cexData || Object.keys(cexData).length === 0) {
            ...
            $(`#${cellId.toLowerCase().replace(/\W+/g, '')}`).html(`
                <div class="pink small">⚠️ ${cexName} TIMEOUT</div>
            `);
            return;
        }
        ...
        try {
           ...
           switch (dexName) {
                ...
                // case 'Magpie':
                //      handleResult('Magpie', await fetchWithCountdown(
                //         safeCellId,dexName,
                //         () => DEXAPIs.getMagpiePrice(inputContract, outputContract, rawAmountIn, network),
                //         timeoutLimit
                //     ));
                //     break;
...
//                console.warn(`⛔ Tidak ada fallback tersedia untuk ${dexName}`);
    ...
    CellResult(token, cexInfo, dexInfo, direction, cexName, dexName) {
        ...
        if (!dexInfo || dexInfo.error || dexInfo.isFallbackLoading || !dexInfo.normalizedOut || dexInfo.normalizedOut <= 0 || dexInfo.hasilUSDT <= 0) {
            ...
            if (dexInfo?.isFallbackLoading) {
                content = `
                    <div class="price-info text-center text-warning small">⌛ Via Swoop...</div>
                    <div class="fee-info">&nbsp;</div>
                    <div class="pnl-info">&nbsp;</div>
                `;
            ...
            content = `
                ...
                <div class="fee-info" title="SWOOP: ${errorMsg}">⚠️</div>
                <div class="pnl-info">&nbsp;</div>
            `;
        ...
        // if (!dexInfo || dexInfo.error || dexInfo.isFallbackLoading) {
        // let content = '';
        // if (dexInfo?.isFallbackLoading) {
        //         content = `
        //             <div class="price-info text-center text-warning small">⌛ Via Swoop...</div>
    ...
    `.trim();
        ...
        if (pnlNetto > 0) {
            ...
            const directionLabel = direction === 'cex_to_dex'
                ? `<span class="ps-1 pe-1 fw-bold ${cexBadgeColor} fs-9">${fromSide.toUpperCase()}</span>&nbsp;<span class="text-success fw-bold">[${fromSymbol}] </span> on <span class="${chainColor} fs-8 fw-bold">  ${shortChain}</span>`
                : `<span class="ps-1 pe-1 fw-bold ${cexBadgeColor} fs-9">${toSide.toUpperCase()}</span>&nbsp;<span class="text-danger fw-bold">[${toSymbol}] </span> on <span class="${chainColor} fs-8 fw-bold">  ${shortChain}</span>`;

            const signalText = `<a href="#${rowId}" class="text-decoration-none text-dark">
                ${directionLabel}: <span class="fs-7 fw-bold" style="color:#18b01e;">${pnlNetto.toFixed(2)}$</span>
            </a>`;
            ...
            // var audio = new Audio('sinyal.mp3');  // Ganti dengan path file suara yang sesuai
            // audio.play();                    
            ...
            const card = $(`
               <div class="col-6 col-md-4 col-lg-2">
                    <div class="card border shadow-sm h-100 rounded-top no-rounded-bottom">
                        ...
                        <!-- HEADER: putih -->
                       <div class="card-header px-2 py-1 d-flex justify-content-center align-items-center border-bottom-0 biru" style="min-height: unset;">
                            <div class="text-center fw-semibold text-uppercase ${textColor}"
                                ...
                                ${dexId}
                                <i class="bi bi-caret-down-fill toggle-icon ms-1" id="icon-${dexId}"
                                    data-bs-toggle="collapse"
                                    data-bs-target="#${collapseId}"
                                    aria-expanded="true"
                                    aria-controls="${collapseId}"
                        ...
                        <div class="card-body  p-1 pt-1 pb-1  bg-white">
                            <div id="${collapseId}" class="collapse show">
                                <ul id="${listId}" class="mb-0 small text-center"></ul>
                            </div>
                        </div>
    ...
    createTokenDetailContent(token, cex) {
        ...
        const tokenSC = `<a href="${url2.tradeLink}" class="fs-7 bg-light ${chainColor} px-1 mx-2" target="_blank">${tokenSymbol}</a>`;
      ...
      return `
            <div>
                ...
                <span class="d-block mb-0">                   
                    <button class="btn btn-outline-danger btn-xs" onclick="app.deleteToken('${token.id}')" title="Hapus">
                        <i class="bi bi-trash"></i>
                    </button>
                    <strong>${token.modalCexToDex}$ ⇔ ${token.modalDexToCex}$</strong>
                    <button class="btn btn-outline-primary btn-xs" onclick="app.editToken('${token.id}')" title="Ubah">
                        <i class="bi bi-pencil"></i>
                    </button>
                </span>
                ...
                <span class="d-block mb-0 fw-bold">
                    <span>${tokenSC}</span><a href="${stokTokenLink}" target="_blank"><i class="bi bi-wallet"></i></a> 
                    VS 
                    ${pairSC} <a href="${stokPairLink}" target="_blank"><i class="bi bi-wallet"></i></a>
                </span>
                ...
                <span class="d-block fw-bold fs-9">
                    ${linkOKDEX} ${linkLIFI} ${linkUNIDEX} ${linkDEFIL}  ${link1INCH}
                </span>
            </div>
            `;
    ...
    showAlert(message, type = 'info') {
        ...
        const alertHtml = `
            <div id="${alertId}" class="alert alert-${type} alert-dismissible fade show position-fixed d-flex align-items-center justify-content-between"
                style="top: 2%; left: 50%; transform: translateX(-50%); z-index: 9999; min-width: 70%; max-width: 90vw; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-right: 3rem;">
    ...
    tryAutoScrollToRow(firstToken) {
        ...
        if (!isAutoScroll) return; // ← tidak scroll jika checkbox tidak dicentang
